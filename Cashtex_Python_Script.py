...
# Laden der Anfangswerte für die DB-Verknüpfung
...
# Festlegung der leeren Eingabefelder für die Berechnungsseite
...
# Werte in Euro-Form bringen

def format_euro(value):
    return f"{value:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")


# Top 5 Unternehmen eines ETFs aus der DB holen
...
# Alle Benutzer aus der DB erfassen
...
# Benutzer aus der DB heraussuchen
...
# Sparplan des gewählten Benutzers aus der DB laden
...
# Sparplan des eingeloggten Benutzers in der DB speichern oder aktualisieren
...
# Diagramm der Kapitalentwicklung erstellen und speichern (nur wenn Daten vorhanden sind)
    ...
    try:
        ...
        if saving_level not in SPARLEVEL:
            ...
            for year in range(1, years + 1):
                ...
                # Jährliche Werte für die Grafik speichern
...
# Aktualisierung der Seite für Anteige der Ergebnisse und Fehlermeldungen
...
# Hauptseite wird geladen und Berechnungen durchgeführt, falls Daten vorhanden sind

@app.route("/berechnen", methods=["GET", "POST"])
def berechnen():

    # Überprüfen, ob ein Benutzer eingeloggt ist und entsprechende Werte setzen, können auch leer sein wenn niht eingeloggt
...
# Unterscheiden zwischen Laden von bestehenden Werten, speichern oder die eingebenen Werte berechnen 
    ...
    if request.method == "POST":
        ...
        if action == "save_current":
            ...
            # Werte aus der Seite zwischenspeichern
    ...
    # Dazugehörige Grafik nach Berechnung speichern
...
#Speichert die vollständig eingegebenen Werte wenn eingeloggt oder leitet zu registrieren weiter
...
# Registrierung eines neuen Benutzers oder Aktualisierung des Sparplans eines bestehenden Benutzers
...
def registrieren():
    try:
       ...
       # Speicherung der Eingaben
        ...
        #Erstellung eines neuen Nutzerprofils
...
# Seite für die detaillierte Kapitalentwicklung mit Diagramm
